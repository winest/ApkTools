...
function TraverseAndRenameApkToVersion( aInFolder , aOutFolder , aLogFolder )
{
    ...
    {
        if ( false == TraverseAndRenameApkToVersion( enumFolder.item().Path , aOutFolder , aLogFolder ) )
        {
            return false;
        }
    }

    CWUtils.DbgMsg( "VERB" , "RenameApkToVersion" , "Checking apk files in \"" + aInFolder + "\"" , aLogFolder );
    var enumFile = new Enumerator( folder.Files );
    for ( ; ! enumFile.atEnd() ; enumFile.moveNext() )
    {
        if ( enumFile.item().Name.match(/.+\.apk/) )
        {
            CWUtils.DbgMsg( "VERB" , "RenameApkToVersion" , "Renaming " + enumFile.item().Name , aLogFolder );
            var info = new CPackageInfo();
            if ( false == GetPackageInfo( enumFile.item().Path , info ) )
            {
                CWUtils.DbgMsg( "ERRO" , "RenameApkToVersion" , "GetPackageInfo() failed for " + enumFile.item().Path , aLogFolder );
                return false;
            }
            var dstName = info.Name + "-ver" + info.Version + ".apk";
            var dstPath = aOutFolder + "\\" + dstName;
            CWUtils.DbgMsg( "INFO" , "RenameApkToVersion" , enumFile.item().Name + " => " + dstName , aLogFolder );
